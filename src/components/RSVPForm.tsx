        ...
        >
          ...
          {isSuccess ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6 relative z-10 text-left">
              ...
              {/* Field 2: Companions Count (Locked to 1 Pass) */}
              <div>
                <label className="block text-[10px] uppercase tracking-[0.2em] font-bold text-[#8A9A5B] mb-2">
                  {lang === "es" ? "Pases Permitidos" : "Allowed Passes"}
                </label>
                <div className="w-full px-5 py-4 rounded-sm border border-[#dfb559]/30 bg-[#faf4e3]/30 text-[#3a4b3d] text-xs sm:text-sm font-semibold flex items-center justify-between">
                  <span>
                    {lang === "es" ? "1 Pase Personal" : "1 Personal Pass"}
                  </span>
                  <span className="text-[9px] uppercase bg-[#8A9A5B] text-white font-bold px-2.5 py-1 rounded-full tracking-wider">
                    {lang === "es" ? "Individual" : "Single"}
                  </span>
                </div>
                <p className="text-[11px] text-[#666] font-serif italic mt-2.5 leading-relaxed">
                  {lang === "es" 
                    ? "* Esta invitación es válida exclusivamente para un (1) pase personal por invitado." 
                    : "* This invitation is valid exclusively for one (1) personal pass per guest."}
                </p>
              </div>
